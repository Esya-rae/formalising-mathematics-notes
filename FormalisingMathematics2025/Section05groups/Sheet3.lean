...
example (a b c : G) (ha : a ∈ H) (hb : b ∈ H) (hc : c ∈ H) :
    a * b⁻¹ * 1 * (a * c) ∈ H := by
  have h1: b⁻¹ ∈ H := by exact (Subgroup.inv_mem_iff H).mpr hb
  have h2: 1 ∈ H := by exact Subgroup.one_mem H
  have h3: a * b⁻¹ ∈ H := by exact H.mul_mem ha h1
  have h4: a * b⁻¹ * 1 ∈ H := by exact H.mul_mem h3 h2
  have h5: a * c ∈ H := by exact H.mul_mem ha hc
  exact H.mul_mem h4 h5
